 ...
 * /predict and /explain endpoints in parallel, and renders the results.
 */
import { useState } from "react";

import { ApiError, explain, predict } from "../api/client";
import type {
...
export function PatientForm() {
  ...
  const [prediction, setPrediction] = useState<BatchPredictionResponse | null>(null);
  const [explanation, setExplanation] = useState<BatchExplanationResponse | null>(null);
  ...
  async function handleSubmit(event: React.FormEvent): Promise<void> {
    ...
    try {
      // Fire both API calls in parallel; wait for both to complete.
  ...
  return (
    <div className="space-y-6">
      ...
      {status === "success" && prediction && explanation && (
        <>
          <PredictionResultPanel prediction={prediction.predictions[0]} />
          <ShapBarChart explanation={explanation.explanations[0]} />
        </>
      )}
    </div>
  );
